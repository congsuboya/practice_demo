...
export function DrawBubbleYValueView(valueInterval, svgCanvasHeight, viewHeight, maxNum, minNum) {
    let valueList = [];
    let valueNum;
    for (let i = 0; i <= valueInterval; i++) {
        valueNum = maxNum * (1 - i / valueInterval);
        if (zeroHide && i == valueInterval) {
            continue;
        }
        valueList.push(<Text style={{
            marginTop: i == 0 ? 5 : (svgCanvasHeight / valueInterval - 10),
...
export function DrawXValueView(valueInterval, svgCanvasWidth, viewWidth, maxNum, full = false) {
    let valueList = [];
    let valueNum;
    let marginLeft = 0;

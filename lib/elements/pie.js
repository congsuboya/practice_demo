...
export default class Pie extends React.Component {
    ...
    CalculationDegrees(pieData, pieR) {
        ...
        pieData.map((item, index) => {
            ...
            xList.push(cx + pieR * Math.cos(pieDeg * Math.PI / 180));
            yList.push(cy - pieR * Math.sin(pieDeg * Math.PI / 180));
            pathViews.push(<Path key={index} fill={this.props.colors[index]} d={`M${cx} ${cy},L${xList[index]} ${yList[index]},A${pieR} ${pieR}, 0 ${item > allData / 2 ? '1' : '0'} 0 ${xList[index + 1]},${yList[index + 1]},Z`} />)
        });
        return pathViews;
    }

    ...
    renderLineItem() {
        ...
        series.map((mapItem, index) => {
            mapItem.data.map((innerItem, innerIndex) => {
                ...
                if (innerIndex == 0) {
                    path = new Path().moveTo(initX, pointY)
                    dStr = `M${initX} ${pointY}`;
                } else {
                    path.lineTo(initX, pointY)
                    dStr = `${dStr} L${initX} ${pointY}`;
                }
            });
            lineViewList.push(<Shape d={path} strokeWidth='1' stroke={ColorList[index]} />)
        });
                ...
                >
                    <View style={{ flex: 1, backgroundColor: 'white', height: viewHeight, width: svgWidth }}>
                        <View style={{ flex: 0, backgroundColor: 'white' }}>
                            < Surface width={svgWidth} height={svgHeight}>
                                {this.renderLineItem()}
                            </Surface>
                        </View>
                        <TouchableWithoutFeedback

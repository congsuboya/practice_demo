    ...
    renderPerLineView(index, series, perLength, perRectHeight, barCanvasHeight, intervalNum, negaNumInterval, maxNum, valueInterval) {
        ...
        series.map((mapItem, innerIndex) => {
            ...
            if (middleNum != undefined) {
                ...
                nextLastNum = mapItem.data[index + 2];
                ...
                if (nextLastNum != undefined) {
                    nextLastNum = nextLastNum + offsetNum;
                }


                if (firstNum == undefined && lastNum != undefined && nextLastNum != undefined) {
                    let y1 = barCanvasHeight - middleNum * perRectHeight;
                    let y2 = barCanvasHeight - lastNum * perRectHeight;
                    let y3 = barCanvasHeight - nextLastNum * perRectHeight;
                    path = new Path().moveTo(initX, y1).curveTo(initX * 1.32, ((y2 - y1) * 0.16 + y1), initX * 2.36, (y2 - (y3 - y1) * 0.16), initX * 3, y2);
                } else if (firstNum != undefined && lastNum != undefined) {
                    let y1 = barCanvasHeight - firstNum * perRectHeight;
                    let y2 = barCanvasHeight - middleNum * perRectHeight;
                    let y3 = barCanvasHeight - lastNum * perRectHeight;
                    path = new Path().moveTo(-initX, y1).curveTo(initX * -0.68, ((y2 - y1) * 0.16 + y1), initX * 0.36, (y2 - (y3 - y1) * 0.16), initX, y2);
                } else if (lastNum == undefined && firstNum != undefined) {
                    path = new Path().moveTo(-initX, barCanvasHeight - firstNum * perRectHeight).lineTo(initX, barCanvasHeight - middleNum * perRectHeight);
                } else if (firstNum == undefined && lastNum != undefined && nextLastNum == undefined) {
                    path = new Path().moveTo(initX, barCanvasHeight - middleNum * perRectHeight).lineTo(3 * initX, barCanvasHeight - lastNum * perRectHeight);
                } else if (firstNum == undefined && lastNum == undefined) {

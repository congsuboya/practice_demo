...
const watchStartEle = function* watchStart(getState) {
    ...
    while (true) {
        ...
        // console.log('actionsfsdfsdf-action', action);
        // console.log('actionsfsdfsdf-state', state)
    }
}

const watchResetEle = function* watchReset() {
    while (true) {
        yield take(types.RESET)
        yield put(actions.stop());
    }
}

const timerEle = function* timer() {
    try {
        while (true) {
            // call: 有阻塞地调用 saga 或者返回 promise 的函数，只在触发某个动作
            yield call(delay, 1000);
            // put: 触发某个action， 作用和dispatch相同
            yield put(actions.runTime());
        }
    } finally {
        if (yield cancelled()) {
